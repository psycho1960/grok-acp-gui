...
/// Production dispatcher with MOD-ARTIFACTS enabled. The compatibility
/// wrapper above keeps existing bridge tests/fakes source-compatible while
/// making attachment operations fail closed when no managed service exists.
pub async fn execute_impl_with_artifacts(
    repo: &dyn Repository,
    runtime: &dyn crate::modules::agent_runtime::AgentRuntime,
    task_runtime: &dyn TaskRuntime,
    ...
    execute_impl_inner(repo, runtime, runtime, task_runtime, Some(artifacts), raw).await
}
...
async fn dispatch(
    ...
    match &cmd {
        ...
        DesktopCommand::ArtifactImport(payload) => {
            ...
            match artifacts.import_images(repo, &payload.task_id, &payload.paths) {
                Ok(imported) => DesktopResult::ok(serde_json::json!({ "artifacts": imported })),
